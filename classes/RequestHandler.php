...
class RequestHandler
{
    ...
    function saveField()
    {
        ...
        if ($path = app()->request(REQUEST_PATH)) {
            ...
            echo app()->jsonMessage(RESULT_OK, "ok1", $add);
        } else
    ...
    function importPersons()
    {
        app()->requireLoginJson();
        //if ($path = app()->request(REQUEST_PATH)) {
            //require_once WFW_ROOT . "phpexcel/Classes/PHPExcel/IOFactory.php";
            require_once DOC_ROOT . "/3rd/phpexcel/Classes/PHPExcel/IOFactory.php";
    ...
    //  Read your Excel workbook
        ...
        $arr = array();
        $policyId = app()->request("policyId");
        if(intval($policyId) > 0) {
